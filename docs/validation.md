...
### 2026-09-12：公式 View 复用与 style 空定义上下文

前端增量合并改为比较每个公式编辑前后的实际源码，不再仅因 Typst 重解析覆盖相邻公式就请求 analyze_formula。宏定义变化仍更新后续投影；公式外的作用域分隔符变化保守保留重解析失效，避免同一调用沿用已不可见的宏绑定。区间移动仍更新 Raw 的 UTF-8 源码位置。

投影安装不再直接使全部公式 Box 失效，保留存活 View 的排版缓存；节点使用的字形或附件数据变化时标记该 View 的绘制版本。style 的请求和缓存查询统一使用空 definitions，按调用表达式与 display 复用；宏定义编辑不清除此字形缓存，同键在途请求合并，切换文档的旧字形回包不再覆盖新请求。

最终完整 Qt 离屏测试 **103 项通过（50.543 秒）**。新增覆盖同段正文编辑复用 View/Box、只重建实际改变的公式、退出编辑复用布局、中文/emoji 前缀下 Raw 区间平移、跨公式字形共享及在途去重、宏作用域开闭后的重新分析。真实适配器验证前面存在错误公式和公式位于内容块时，空定义上下文的 bold(x) 均返回正确字形。仅修改 Python 前端与文档，无需重新构建 Rust 二进制；未使用 computer use，未打开可见桌面窗口。

### 2026-09-12：完成 style / raw_macro 八项问题修复

延续上一轮已完成的空 definitions、按公式复用 View 与字形在途去重，本轮补齐以下路径：

- style 字形失败保存为独立状态，显式刷新才重试；合法空字形与等待状态区分。回包只重排使用该表达式的公式。
- 宏模板绑定直接保留实参 View 与光标，移除将实参写成文本再无会话重解析的路径；空字符串和下划线参数名同时覆盖。
- 模板 raw_macro 分开保存绑定后的显示拼写和定义中的定位拼写。取图请求携带调用区间与重复展开次数，适配器在内存源码中标记两层边界，按实例匹配图片。不可见 metadata 防止单片段的内外标签在数学 IR 中被合并。
- host 始终给内部片段提供定位信息，前端在光标进入调用后选择内部片段，避免与外部整图重叠。位置移动同步更新请求范围，实例缓存以公式身份和调用相对位置复用。
- raw_macro 取图失败显示源码框。左右键可进入源码草稿，Esc 恢复原 MacroCall，确认后解析并写回；style 失败也可修复。
- LSP 诊断统一转换到 UTF-8 字节位置，正文 View 与活动 View 同步错误状态，修复或清除诊断后解除标记；警告不当作渲染失败。

最终验证：Rust 默认全套 **150 项通过、6 项环境依赖测试忽略**；真实适配器 **22 项通过**；独立 release 后端与适配器上的 Qt 离屏全套 **109 项通过（56.874 秒）**。端到端测试包含不同实参的不同图片、同次调用重复展开、内部取图、可见参数光标、失败草稿修复、中文/emoji 多行诊断，以及插入正文后的实例缓存复用。

用户在开发期间继续编辑文档，examples/tutorial.typ 的外部修改保留。编译前已告知，生成了单独命名的 target/server/release/typformula-review.exe 与 target/adapter/release/typformula-layout-review.exe；target/start-fixed.cmd 指定这两个已验证版本启动。没有关闭用户窗口，没有使用 computer use，所有窗口测试都运行于 offscreen 平台。

### 2026-09-12：正文诊断、悬停与跳转定义

正文编辑器、分栏与源码栏接入语言帮助：错误/警告波浪线、诊断提示、鼠标悬停与 Ctrl+K Ctrl+I 符号说明、F12/右键跳转定义。语言位置从 Qt 投影还原到源码，兼容中文、emoji 和折叠公式；旧悬停回包在移动鼠标或修改文档后丢弃。同文件定义在源码栏定位，跨文件定义另开窗口，保留原文档的未保存源码与撤销历史。

实测定位出诊断缺失的真正原因：Windows canonicalize 返回的 verbatim 路径直接传入 pinMain 后，Tinymist 会把它与 didOpen URI 对应的普通路径当成不同文档，从而在正确的内存诊断之后发布磁盘诊断。后端现将 pinMain 和实时预览命令的路径统一为 URI 对应的正常路径；不存在于磁盘的新文档也能得到正确诊断，不创建占位文件。

最终 Rust 默认测试 **151 项通过，6 项环境依赖测试默认忽略**；另运行真实 Tinymist 实时预览测试通过。独立 release 修复版上的完整 Qt 离屏测试 **115 项通过（57.037 秒）**，包括真实补全、hover、definition、内存诊断、跨文件跳转与编辑内容保留。此次未使用 computer use，没有操作用户当前窗口，也未改动用户迁移到 docs 下的教程文档。

### 2026-09-12：多行上下标与值宏命令确认

旧版复现：单独确认 rme/rmi 会被追加为 rme()/rmi()；在较长命令表达式中确认 rmi + 1，虽然源码仍写 rmi，View 却出现四个空参数槽。前者来自把所有可展开绑定都当成函数，后者来自把 MacroCall 的重复参数形状套用了四格矩阵的默认补槽逻辑。现在按 function 区分内容值与函数，只为配置中的结构命令补默认槽；用户函数使用定义参数数，零参数函数和未知的显式空调用都不再被补四格。

多行公式原先因多了一层 Multiline 而漏掉附件请求。投影现在把对齐单元格视为保持数学样式的顶层范围，发送每个顶层上下标的请求。适配器接受完整的封闭文档上下文和目标公式区间，在目标位置求值待查表达式，保留内容块里的自定义算符定义；同一块含其它公式时按标记选择目标，避免误取最后一个公式。

最终验证：Rust 默认测试 **154 项通过、6 项忽略**；真实适配器 **23 项通过**；独立 release 组合的 Qt 离屏全套 **117 项通过（62.592 秒）**。新增用例包含内容值的裸命令/组合表达式/直接插入、真实 LSP 补全、零参数函数、标准结构命令、多行附件元数据、内容块内 sum 的 limits/scripts 区分、自定义算符以及修正 rme() 后整段公式的真实编译。源文档保持不变，独立修复版已更新，仍可用 target/start-fixed.cmd 启动。

### 2026-09-12：新建上下标的空槽可见性
...
### 2026-09-13（续五）：删除行/列，以及只在列表里出现的「列表工具栏」

用户要求：给现有的"新建行/新建列"配上"删除行/删除列"，四件事一起收进一个「列表工具栏」，放在主工具栏下方（选定：第三条工具栏，位于「数学」下方），**激活列表时才显示**。两个岔路口由用户当场选定：删除删**光标所在**的行/列（LyX 的做法）；多行公式删到只剩一格时**拒绝并提示**（LyX 在只剩一行/一列时也是静默返回，这里改成出声）。

**一、一处"功能撞上不变量"的地方，值得单独记。** 最初我按"剩下至少两格"写的底线，实测发现它**不够**：`a & b \\ c` 删掉第一行后剩一格可见内容加一格补齐，`row_lengths = [1]`，写回源码是 `c`——`&` 和 `\\` 都没了，再解析回来只是一个普通公式，而树里还留着 `Multiline`（`tests/round_trip.rs` 守的正是"回写的唯一安全网"）。所以真正的底线不是格子数，而是**必须还有一个 `&` 或 `\\`**：行数 ≥ 2，或者只剩一行时那一行宽度 ≥ 2。矩阵没有这条底线，因为 `mat(a)` 是有拼写的。牙齿验过：把这条判据改成恒假，`an_alignment_keeps_a_marker_or_says_why_it_cannot_shrink` 立刻失败，实测值正是 `left: "b"`——就是上面那个退化成普通内容的形态。

**二、前端那个坑：标记是 `_active`，不是 `active`。** `mark_active_path` 把光标那个 stop 的祖先打上 `_active`（stop 自己带 `active`），我第一版 `active_list` 查的是 `active`，于是永远返回 None、工具栏永不出现。第二版按"第一个带标记的节点"返回，仍然错——那样拿到的是**父节点的** grid（进入该节点时就该把它自己算进去），而且嵌套列表要的是**最内层**。定稿：沿标记链往下走，"最深的被标记节点的最内层列表祖先"。选区内核心不标任何活动 stop（`response` 只在没有 anchor 时才 `activate`），此时按 `selected` 回溯同一问题，免得选一段就闪一下。

**三、`isVisible()` 在没 show 的窗口上恒为假。** 前端用例第一版全红，原因不是逻辑而是 Qt 的语义：`isVisible` 要整条祖先链都可见才算。实测 `window.show()` 之后同一次操作得到 `visible: True`，于是用例显式开窗（离屏开窗既有用例也在用）。

**四、又一次踩到 release 二进制过期。** 前端用例跑的是 `target/server/release/typformula.exe`，我只跑了 debug 构建，于是 `remove_row` 反序列化失败、核心原样不动、消息为空——症状看起来像"前端没接上"。按文档构建 release 后端后立刻全绿。这与「续三」记的适配器过期是同一类问题：**改核心/适配器后必须按 AGENTS.md 那两条 `cargo build --release` 重建，跑测试不算**。

**五、命名。** 两件既有命令的可见标题从「矩阵增加行/矩阵增加列」改成用户口述的「新建行/新建列」，新的两件是「删除行/删除列」，四条都仍在「数学」菜单里（快捷键按标识符 `addRow`/`addColumn`/`removeRow`/`removeColumn` 存储，改名不影响已有配置）。副作用是 `docs/tutorial.typ:152` 那句"可以点击「矩阵增加行(列)」选项卡"outdated；该文件是用户自己的，未改。
