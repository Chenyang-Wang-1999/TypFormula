...
Raw SVG 按可见公式/活动公式按需加载。**取图的节点有两种**：`raw`（编辑器不建模的片段）与 `raw_macro`（它拒绝展开的调用，见 [architecture.md](architecture.md)）——后者在光标**不在**节点里时画成整段调用的一张图，光标进去后换成宏名与参数槽；两种都靠 `text`（源码）+ `edit`（光标）定位。因此 `window.raw_fragments` 做的是**带祖先的**遍历，会跳过塌缩调用内部的片段：整段调用是一张图，实参槽里的片段在光标进入前不单独取图，否则既白编译一遍又与那张整图重复。每个 Raw 的源码区间由 `Document::annotate` 给出：规范序列化只用来估算位置，最终以"该区间里恰好是这段文本"为准，因此按自己习惯书写的公式（`sum_(n=0)` 而规范写法是 `sum_(n = 0)`）同样能出图。结构编辑中，Raw 的缓存身份独立于源码字节位置，编辑相邻字符不会重新请求已有 SVG；修改脚标时保留基底 Raw，离开修改过的脚标槽后只失效该基底。**独立的例外是 attach 里的片段**（见下一段）：基底的盒子会跟着它自己的脚标变（`stretch(->)^x` 的宽度来自 `x`），所以那类片段按"所在 `script` 的形状"缓存，改脚标就重取。新 Raw、Raw 源码改变、正文/定义重建及显式刷新会按需生成新的结果。桌面端不再自动编译整页预览。
...
合法但尚未结构化支持的数学节点会作为 Raw 保留，整个公式仍可进入编辑器。这类片段不是解析失败：它们由 Typst 渲染成图像（`sum`、`integral`、`dif`、`lr(x, size: #100%)` 等），只有排版结果为空白或后端给不出可见结果的片段（例如 `quad`）才退回源码显示——那一类会画成暖色底 + 红色虚线框，提示"这里有一个还没有图像的片段"，并且可以用左右键进入源码修复，修好按 Enter 提交（若解析不成公式，则按片段源码提交）。位于不可展开 `let` 内容中的公式按照需求保持源码模式；它不是 Typst 解析失败。源码下方的红色波浪线和悬停提示现在会区分“不可展开 let”与真正的结构转换错误。
...
- 公式里的节点会按语言服务（Tinymist）的诊断标红：只标**引擎必须求值的那些节点**（带 `render_id` 的，也就是内核画不出、退成整段图的调用），**可编辑的格子永远不标**，所以参数打到一半不会变红。画法与"图的请求被拒绝的片段"完全相同——源码、暖底、虚线框，双击可进源码修。文档级诊断（例如缺 import）不标在任何公式上，只出现在状态栏。
