...
- 普通模式：进入公式框时，默认处于普通模式。在普通模式下，输入的字符之间默认都带一个空格。例如：依次键入 `f,r,a,c`，得到： $f r a c$  (源码：`$f r a c$`)。
  #annote()[两个例外：
  - 数字：由于 Typst 对数字的特殊处理，普通模式下输入数字会默认在数字之间不加空格。但是，仅限于数字。在数字中间输入 `"."` 还是默认添加空格的。如果你想输入紧凑的小数，请使用命令模式。
  - 除号：输入 `"/"` 时，公式编辑器会自动创建分式，并转成`frac`。
  ]

- 命令模式：在公式框中按下 `"\"` 即可进入命令模式。`LyX` 用户应该会很习惯这种交互逻辑。命令模式和普通的 Typst 文本编辑器没有差别。按回车后，编辑器自动解析。例如，你可以在公式框中按`"\"`进入命令模式，输入`frac(1,2)`，回车，就会得到：$ frac(1, 2) $ 
...
相较于其他可视化编辑器，比如 LyX、TyX、MathML 与 AxMath，TypFormula 采用了一种可扩展的渲染方式。得益于 Typst 强大的实时编译，我们不需要一一复刻 Typst 引擎的所有规则。当编辑器遇到没有记录的命令时，它会调用 Typst 引擎实时编译，并在编辑器内渲染成 svg 块。事实上，我们只内置了 40 种符号和 10 余种宏（见`config`目录），剩下的部分都依赖引擎自己渲染。
...
$ tensor(F, + mu, - nu) = partial^(mu) A_(nu) - partial_(mu) A^(nu) $
...
1. 出现了未知符号，即，不含参，且不在 `config/symbols.json` 中的命令，如 `in`, `RR`。此时，后端会将该内容识别为 "Raw"，然后前端会将它们解析成图片并缓存。一旦生成图片后，就无法再编辑源码内容。但你可以将符号整体删除。
