...
附件查询的顶层范围穿过 Multiline 对齐容器，因为对齐单元格不改变数学样式。分式、上下标内部等嵌套仍不在此范围内。前端附件请求带目标公式的源码区间与完整文档，host 用与取图相同的 `context` 合成上下文（`crates/core/src/context.rs`），把目标公式替换成待查询表达式并标记后再交给适配器。这样保留 `#[…]` 的绑定作用域与公式所在容器的样式，避免直接使用未闭合前缀；标记也防止误选同一块里后面的其它公式。字形请求仍保持空 definitions 的独立策略。适配器在求值后的内容里找那个标记：它沿**元素字段**遍历（`#columns`、`#grid`、列表项、以及文档级 `#show` 把整篇文档放进的那个元素都在字段里），并沿父链重建 `StyleChain`，因为 `math.limits` 这类样式决定答案本身。模板把正文还给 `context { … }` 时（zhaji 的 `note` 默认就是），那段正文要到 realization 才求值，遍历看不到它：此时适配器退到顶层，用**裸表达式**按请求的显示模式再问一次（答案仍来自引擎，只是不再带文档作用域），回包带 `fallback`；前端据此在状态栏说明，而不是静默画成侧挂脚标。定位不到公式以外的失败不重问；请求本身失败时前端同样报出原因。
