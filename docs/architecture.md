...
矩阵的短行补空块至齐行，补出的格子参与显示、导航和回写。进入公式本身仍不改写原始拼写；首次结构编辑后写出完整矩形。普通 `&` 对齐公式仍保留各行原来的列数。增删行列都是核心的动作（`grow_grid`/`shrink_grid`），走同一条结构编辑写回路径，前端只负责在光标进入列表时才把「列表」工具栏摆出来（见 [desktop.md](desktop.md)），判据本身仍只有核心有。
