...
附件查询的顶层范围穿过 Multiline 对齐容器，因为对齐单元格不改变数学样式。分式、上下标内部等嵌套仍不在此范围内。前端附件请求带目标公式的源码区间与完整文档，host 截断到包含该公式的完整顶层语法节点，适配器只在内存中将目标公式替换成待查询表达式并标记。这样保留 `#[…]` 的绑定作用域，避免直接使用未闭合前缀；标记也防止误选同一块里后面的其它公式。字形请求仍保持空 definitions 的独立策略。
