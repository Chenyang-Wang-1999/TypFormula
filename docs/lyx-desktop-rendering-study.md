...
5. 每次输入后的后台任务仍会启动宏预热、attachment 查询和语义高亮。它们虽有 550 ms 防抖，仍可能与继续输入争用 Rust 服务、Python 主线程或 Qt 更新。
...
宏预热只在新增或修改可展开 `let` 时运行。attachment 只在表达式或其可见作用域依赖改变时运行。Raw 仅在首次进入可见区、Raw 自身源码变化，或影响它的 `let` 发生变化时请求引擎。
...
- 在公式外连续输入时：重建公式数为 0，Raw 请求数为 0，宏预热数为 0。
