...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    std::fs::create_dir_all(&services.workspace)?;
    println!("Visual Typst · http://127.0.0.1:{port}\n{}\nCtrl+C 停止", services.status());
    for mut request in server.incoming_requests() {
        ...
        if url.starts_with("/api/") {
            ...
            std::thread::spawn(move || {
                let result: Result<serde_json::Value, String> = (|| {
                    if url == "/api/files" && request.method() == &Method::Get { return visual_typst_core::workspace::files(&services.workspace); }
                    ...
                    let mut body = String::new(); request.as_reader().take(4*1024*1024+1).read_to_string(&mut body).map_err(|e| e.to_string())?;
        ...
        let asset = match url.as_str() {
            ...
            "/editor.bundle.js" => Some(("editor.bundle.js", "text/javascript; charset=utf-8")),
